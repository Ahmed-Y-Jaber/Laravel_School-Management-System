...
class AddParent extends Component
{
    ...
    use WithFileUploads; // يجب تضمينه


    public $successMessage = '';
    public $catchError, $updateMode = false, $photos,$show_table = true,$Parent_id;
    /* يجب ان نعمل public $photo;
    $show_table = true في بداية الصفحة اعرض جدول
    $Parent_id بتلزم ناخدها لاكمال التعديل
*/
    // الصفحة المطلوب يقف عليها من البداية رقم 1
    ...
    // real time validation يتم وضعها قبل render
    // ايضا تتم الترجمة تلقائيا من لارافيل من خلال ملف
    // validation
    // داخل اللغة .. مع ترجمة بعض الحقول داخله
    ...
    public function showformadd(){
       ...
       public function submitForm(){
        ...
        try {
            ...
            // this بدل كلمة request in controller
            // $Sections->Grade_id = $request->Grade_id;
            $My_Parent->Password = Hash::make($this->Password);
            // يجب تعريف ترجمة الحقول في model
            ...
            $My_Parent->save();
                ...
                // اضافة الصورة
                // if (!empty ليس اجباري اضافة المرفقات
            if (!empty($this->photos)){
                foreach ($this->photos as $photo) {
                 /*   يتم حفظ الصور بخطوتين
                    1- حفظ الصورة على الستوريج
                    2- حفظ رابط الصورة على قاعدة البيانات
                    الامر التالي يخص الستوريج*/
                    $photo->storeAs($this->National_ID_Father, $photo->getClientOriginalName(), $disk = 'parent_attachments');
                   /* $this->National_ID_Father افتح فولدر برقم هوية الاب
                   $photo->getClientOriginalName() هات اسم الصورة
                   $disk = 'parent_attachments' انا بدي مرفقات الاب تكون منفصلة عن مرفقات الام او الطلاب
                   وهذه بنجيبها من  الكود الذ ضفناه في
                   config- Filesystem  */
            ...
            /*
                لو لقي اخطاء من داخل ال try
                يظهر رسالة جميلة

                تم تعريف ال  $catchError
                في بداية الصفحة
            */
